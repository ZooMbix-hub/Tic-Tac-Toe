...
const scriptsDb = require("./helpers/db");
const scriptsOther = require("./helpers/other");
...
io.on("connection", (socket) => {
    ...
    socket.on("move_player_server", (room, squares, playersRoom, time) => {
        ...
        if (draw || winner !== null) {
            ...
            if (draw !== null) {
                scriptsDb.updateUser(historyArr[0].login, "draw");
                scriptsDb.updateUser(historyArr[1].login, "draw");
            } else {
                for (let index = 0; index < 2; index++) {
                    if (historyArr[index].winner == true) {
                        scriptsDb.updateUser(historyArr[index].login, "victory");
                    } else {
                        scriptsDb.updateUser(historyArr[index].login, "loss");
                    }
                }
            }
