...
module.exports.updateUser = async function (login, result) {
    const user = await User.findOne({ where: {login: login} }).catch((error) => console.log(error));

    switch (result) {
        case "victory":
            await user.increment(['countGames', 'countGamesWin'], {by: 1});
            break;
        case "loss":
            await user.increment(['countGames', 'countGamesLos'], {by: 1});
            break;
        case "draw":
            await user.increment({ 'countGames': 1 });                
            break;
    }
}